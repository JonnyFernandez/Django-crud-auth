...
from django.contrib.auth import (
    ...
    logout,
    authenticate,
)  # este metodo me ayuda a crear cookies para guardar en el navegador
from django.db import IntegrityError  # para menejar esepciones de la db
...
@login_required
def tasks(request):
    # all_task = Task.objects.all() #trae todos las tasl
    all_task = Task.objects.filter(
        user=request.user, datecomplete__isnull=True
    )  # filtra las trask por usuario,

    return render(request, "task.html", {"tasks": all_task})
...
def create_task(request):
    if request.method == "GET":
        ...
        # print(request.POST)
        # tengo 2 formas de guardar los datos, usando el modelo de Task, o usando el formulario TaskForm
        try:
            ...
            return render(
                request,
                "create_task.html",
                {"form": TaskForm, "error": "Ingresar datos validos"},
            )
